...
export interface SortOptionsInterface {
  by?:
    ...
    | Array<
        | string
        | { key: string; descending?: boolean; caseInsensitive?: boolean }
      >;
  descending?: boolean;
...
const compareKeys = (
  ...
  if (key) {
    first =
      isObject(first) || isArray(first)
        ? getObjectDeepKeyValue(first, key as string)
        : first;
    second =
      isObject(second) || isArray(second)
        ? getObjectDeepKeyValue(second, key as string)
        : second;
  }
